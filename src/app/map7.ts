...
plotPolygon(coordinates: [number, number][]) {
  ...
  const closedCoordinates: [number, number][] = [...coordinates, coordinates[0]]; 
  ...
  // Define colors and styles for different keys
  // const keyStyles: { [key: string]: { fillColor: string, strokeColor: string, strokeWidth: number, dashArray?: number[] } } = {
  //     '24SUPPYGAB63C4332': { fillColor: '#ff0000', strokeColor: '#ffcccc', strokeWidth: 2, dashArray: [2, 2] }, // Red with dashed border
  //     '24SUPPYGAB6314332': { fillColor: '#0000ff', strokeColor: '#ccccff', strokeWidth: 2 }  // Blue with solid border
  // };

  // Sort keys based on the number of data points
  const sortedKeys = Object.keys(this.groupedData).sort((a, b) => this.groupedData[b].length - this.groupedData[a].length);
  // Loop through the sorted keys and plot PLTIDs
  // let cropData: any[] = [];
  // sortedKeys.forEach(key => {
  //     cropData = this.groupedData[key];
  //     const styles = keyStyles[key] || { fillColor: '#00dcff', strokeColor: '#FF0000', strokeWidth: 1 }; // Default styles
  ...
  //                 color: item.CLR,
  //                 fillColor: item.CLRFILL,
  //             },
  //         };

  //         this.map!.addSource(`barcode-${item.barcode}`, {
  //             type: 'geojson',
  //             data: squareGeojson as any,
  //         });
  ...
  //         this.map!.addLayer({
  //             id: `barcode-border-${item.barcode}`,
  //             type: 'line',
  //             source: `barcode-${item.barcode}`,
  //             paint: {
  //                 'line-color': styles.strokeColor,
  //                 'line-width': styles.strokeWidth,
  //                 'line-dasharray': styles.dashArray || [1, 0], // Apply dash array if defined
  //             },
  //         });
  //         // Highlight the cell
  //         // this.highlightCell(squareGeojson.geometry.coordinates[0] as [number, number][], `highlight-${item.barcode}`, styles.fillColor, styles.strokeColor, 0.5, styles.strokeWidth,this.groupedData);
  //     });
  // });
  this.drawGridLines(squareCoordinates,this.groupedData);
}
...
drawGridLines(coordinates: number[][],cropData:any) {   
  ...
  this.addTextToCells(cellCenters, cellBoundaries, coordinates as any,cropData); // Add text to cells
}
...
plotGpsPoints(coordinates: number[][]) {
 ...
 drawLine(coords: any[], lineId: string, color: string = '#cceeff', width: number = 1) {
...
// highlightCell(cellBounds: [number, number][], highlightId: string, fillColor: string = '#cceeff', strokeColor: string = 'pink', fillOpacity: number = 0.5, strokeWidth: number = 5, groupedData: any) {
//   // Check if the source already exists
//   if (this.map?.getSource(highlightId)) {
//       this.map?.removeSource(highlightId);
//   }

//   let originalhighlightId = highlightId.slice(10); // eg: "LC001703546"
//   let foundItem = null;
//   let strokeColors = '';
//   let keys = Object.keys(groupedData);

//   for (let key of keys) {
//     const found = groupedData[key].find((item: { barcode: string; }) => item.barcode === originalhighlightId);
    
//     if (found) {
//         foundItem = found;
//         strokeColors = key === keys[0] ? 'brown' : strokeColor; // Apply brown for the first key, default for others
...
//       'layout': {},
//       'paint': {
//           'fill-color': fillColor, // Background color
//           'fill-opacity': fillOpacity // Adjust opacity if needed
//       }
//   });
...
//       'layout': {},
//       'paint': {
//           'line-color': strokeColors, // Border color
//           'line-width': strokeWidth // Border thickness
//       }
//   });
// }
highlightCell(cellBounds: [number, number][], highlightId: string, borderColor: string) {
  if (this.map?.getSource(highlightId)) {
    this.map?.removeSource(highlightId);
  }

  console.log(`Adding source for highlightId: ${highlightId}`);
  this.map?.addSource(highlightId, {
    type: 'geojson',
    data: {
      type: 'Feature',
  ...
  console.log(`Adding fill layer for highlightId: ${highlightId}`);
  this.map?.addLayer({
    id: `${highlightId}-fill`,
    type: 'fill',
    source: highlightId,
  ...
  console.log(`Adding line layer for highlightId: ${highlightId} with color: ${borderColor}`);
  this.map?.addLayer({
    id: `${highlightId}-line`,
    type: 'line',
    source: highlightId,
    layout: {},
    paint: {
      'line-color': borderColor, // Use the passed border color
      'line-width': 3,
    },
  });
}
...
addTextToCells(
  ...
  cropData: { [key: string]: { PLTID: string; TID: string; }[] }
) {
  console.log(cropData);
  const texts = this.plotdata[0].plotinfo; 
  let trialId: string;

  // Determine the key with the highest number of entries
  let maxEntriesKey = '';
  let maxEntriesCount = 0;

  for (const key in cropData) {
    if (cropData.hasOwnProperty(key)) {
      const entriesCount = cropData[key].length;
  ...
  // Assign border colors based on the key with the highest data
  const borderColors: { [key: string]: string } = {};
  for (const key in cropData) {
    if (cropData.hasOwnProperty(key)) {
      borderColors[key] = key === maxEntriesKey ? 'cyan' : 'yellow';
      console.log(`Assigned color for key ${key}: ${borderColors[key]}`);
    }
  }
  ...
  for (const key in cropData) {
    if (cropData.hasOwnProperty(key)) {
      const entries = cropData[key];
      // Highlight the cells for the current key
      entries.forEach((entry: { PLTID: string; }, index: number) => {
        if (index < cellBoundaries.length) {
          console.log(`Highlighting cell for PLTID: ${entry.PLTID} with color: ${borderColors[key]}`);
          this.highlightCell(cellBoundaries[index], `highlight-${key}-${index}`, borderColors[key]); // Pass the color
          totalHighlighted++;
        } else {
          console.log(`No boundary available for entry: ${entry.PLTID} at index: ${index}`);
        }
      });
    }
  }
  console.log(`Total plots highlighted: ${totalHighlighted}`);

  for (const element of texts) {
    trialId = element.TID.length > 12 ? element.TID.substring(8).slice(0, -4) : element.TID;
  }
