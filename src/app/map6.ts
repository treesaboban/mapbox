...
plotPolygon(coordinates: [number, number][]) {
  ...
  const keyColors: { [key: string]: { fillColor: string, strokeColor: string } } = {
      '24SUPPYGAB63C4332': { fillColor: '#ff0000', strokeColor: '#ffcccc' }, // Red
      '24SUPPYGAB6314332': { fillColor: '#0000ff', strokeColor: '#ccccff' }  // Blue
  };

  // Loop through the data and plot PLTIDs
  Object.keys(this.groupedData).forEach(key => {
      const cropData = this.groupedData[key];
      const colors = keyColors[key] || { fillColor: '#00dcff', strokeColor: '#FF0000' }; // Default colors

      console.log(`Key: ${key}, Colors:`, colors);
      ...
      cropData.forEach((item: any) => {
          ...
          // Highlight the cell
          this.highlightCell(squareGeojson.geometry.coordinates[0] as [number, number][], `highlight-${item.barcode}`, colors.fillColor, colors.strokeColor);
      });
  });
...
highlightCell(cellBounds: [number, number][], highlightId: string, fillColor: string = '#cceeff', strokeColor: string = '#66ccff', fillOpacity: number = 0.5, strokeWidth: number = 3) {
  // Check if the source already exists
  ...
  this.map?.addLayer({
      ...
      'layout': {},
      'paint': {
          'line-color': strokeColor, // Border color
